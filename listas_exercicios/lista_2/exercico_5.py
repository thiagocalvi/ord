...
def main():
    ...
    try:
        arq = open(nomeArq, 'rb')
        newArq = open(nomeArq+sys.argv[2], 'wb')
        if sys.argv[2].upper() == "WINDOWS" and sys.argv[3].upper() == "LINUX":
            win_to_linux(arq)

        elif sys.argv[3].upper() == "LINUX" and sys.argv[2].upper() == "WINDOWS":
            linux_to_win(arq)
...
def ler_caracter(nomeArq):
    ...
    return caracter_binario.decode()


def win_to_linux(arq):

def linux_to_win(arq):